...
- **`creative-director`** — gate **CD-PILLARS** (`.agents/docs/director-gates.md`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** — gate **AD-CONCEPT-VISUAL** (`.agents/docs/director-gates.md`)
...
**After identifying biggest technical risks, spawn `technical-director` via Task using gate TD-FEASIBILITY (`.agents/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via Task using gate PR-SCOPE (`.agents/docs/director-gates.md`).**
...
If yes, generate the document using the template at `.agents/docs/templates/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
...
> to `design/gdd/game-concept.md`. Open a fresh active harness session to continue
