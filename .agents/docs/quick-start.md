...
This is a complete coding-agent architecture for game development. It
...
- **Tier 1 (highest reasoning)**: Directors who make high-level decisions
...
- **Tier 2 (balanced reasoning)**: Department leads who own their domain
  - `game-designer`, `lead-programmer`, `art-director`, `audio-director`,
    `narrative-director`, `qa-lead`, `release-manager`, `localization-lead`

- **Tier 3 (balanced or fast reasoning)**: Specialists who execute within their domain
...
Also in `.agents/docs/templates/collaborative-protocols/` (used by agents, not typically edited directly):
...
.agents/
  ...
  rules/                           -- Provider-neutral path-specific standards
...
.claude/
  settings.json                    -- Claude Code adapter hooks and project settings
  agents/                          -- Claude adapter agent definitions
  skills/                          -- Claude adapter slash command definitions
  hooks/                           -- 13 hook scripts (.sh) wired by settings.json
  rules/                           -- Claude adapter copy of 14 path-specific rule files
  docs/
    quick-start.md                 -- Claude adapter copy of this guide
    technical-preferences.md       -- Project-specific standards (populated by /setup-engine)
    coding-standards.md            -- Coding and design doc standards
    coordination-rules.md          -- Agent coordination rules
    context-management.md          -- Context budgets and compaction instructions
    directory-structure.md         -- Project directory layout
    workflow-catalog.yaml          -- 7-phase pipeline definition (read by /help)
    setup-requirements.md          -- Claude adapter setup notes
