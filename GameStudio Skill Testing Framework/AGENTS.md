# GameStudio Skill Testing Framework -- Provider-Neutral Harness Instructions

This folder is the quality assurance layer for the GameStudio skill/agent
...
| File | Purpose |
|------|---------|
| `catalog.yaml` | Master registry for all 182 skills and 55 agents. Contains category, spec path, and last-test tracking fields. Always read this first when running any test command. |
...
- Skill specs: `GameStudio Skill Testing Framework/skills/[category]/[name].md`
- Agent specs: `GameStudio Skill Testing Framework/agents/[tier]/[name].md`
- Catalog: `GameStudio Skill Testing Framework/catalog.yaml`
- Rubric: `GameStudio Skill Testing Framework/quality-rubric.md`
...
pixi        → pixi, pixi-* (PixiJS v8 reference skills)
phaser      → phaser-* (Phaser 4 reference skills)
threejs     → threejs-* (Three.js reference skills)
react       → react-three-fiber-setup, r3f-*
ecs         → ecs-*
typescript  → typescript-*
mobile      → mobile-performance, touch-input-handling, battery-optimization, memory-management
game-systems→ ai-system, audio-system, camera-system, collision-system,
              health-combat-system, input-system, inventory-system, level-system,
              physics-system, spawn-system, ui-system
web         → web-game-foundations, phaser-2d-game
...
2. Read the skill at `.agents/skills/[name]/SKILL.md` (or the active adapter copy such as `.claude/skills/[name]/SKILL.md`)
...
No runtime adapter imports from here. Deleting this folder has no effect on the
GS skills or agents themselves. `/skill-test` and `/skill-improve` will report that
